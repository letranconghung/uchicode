...
def read_MNIST(
    ...
    dataset_root_dir: str = 'mnist_data',
    load_small: bool = False
) -> Union[Tuple[np.ndarray, np.ndarray], np.ndarray]:
    '''
    Returns MNIST data (training, validation, or testing) from dataset_root_dir. If
    load_small is True, returns the small dataset version.
    
    Args:
        dataset_type: (default "training") Type of dataset, one of training,
            validation, or testing.
        dataset_root_dir: (default "mnist_data") Dataset directory.
        load_small: (default False) Flag to load the small dataset.
    
    Returns:
        (X, y) of shapes (n, d, d) and (n).
    '''
    assert dataset_type in ['training', 'validation', 'testing'], \
        'Dataset must be "training", "validation" or "testing"'
    
    if not os.path.isdir(dataset_root_dir):
        # Download data, and make splits if dataset_root_dir does not exist
        # Credits: https://gist.github.com/goldsborough/6dd52a5e01ed73a642c1e772084bcd03
        os.makedirs(dataset_root_dir, exist_ok=True)
        
        for resource, fname in MNIST_RESOURCES:
            path = os.path.join(dataset_root_dir, resource)
            url = f'http://yann.lecun.com/exdb/mnist/{resource}'
            download(path, url, verbose=True)
            unzip(path)
            print(f'Downloaded and unzipped {resource}')

        mndata = MNIST(dataset_root_dir, return_type='numpy')
        ...
        # Create splits
        xTr, yTr, xVal, yVal = create_split(xTr, yTr, split_ratio=0.8)
        
        # Save mnist data
        for m in ['xTr', 'yTr', 'xVal', 'yVal', 'xTe', 'yTe']:
            np.save(os.path.join(dataset_root_dir, f'{m}.npy'), eval(m))
            np.save(os.path.join(dataset_root_dir, f'{m}_sm.npy'), eval(m)[:500])
    ...
    file_name_suffix += ('_sm' if load_small else '')
    
    # Return data
    X = np.load(os.path.join(dataset_root_dir, f'x{file_name_suffix}.npy'))
    y = np.load(os.path.join(dataset_root_dir, f'y{file_name_suffix}.npy'))
    return X, y


def show_MNIST_example(image: np.ndarray, d: int = 28):
    '''
    Renders a given numpy.uint8 1-d array of pixel data as an image.
    
    Args:
        image: 1-d array of pixel data, the last d**2 of which are reshaped
            into a (dxd) image. shape (d**2)
